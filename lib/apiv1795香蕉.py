...
class Spider(Spider):
    ...
    def _extract_video(self, element, category_id=None):
        ...
        try:
            ...
            # 提取标题
            title_elem = element.xpath('./qyg12.js')
            if not title_elem:
                title_elem = element.xpath('.//p[contains(@class, "script")]/text()')
                if not title_elem:
                    title_elem = element.xpath('./qyg13.js')
                    if not title_elem:
                        title_elem = element.xpath('./qyg14.js')
                        if not title_elem:
                            title_elem = element.xpath('./qyg15.js')
                            if not title_elem:
            ...
            # 提取图片
            pic_elem = element.xpath('./qyg16.js')
            if not pic_elem:
                pic_elem = element.xpath('./qyg17.js')
