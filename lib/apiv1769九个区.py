...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            ...
            if page_elements:
                try:
                    last_page = page_elements[-2].xpath('./qyg0.js')[0]  # 获取"尾页"前一个元素
    ...
    def _extract_video(self, element):
        ...
        try:
            ...
            link = element.xpath('./qyg1.js')[0]  # 获取a标签的href属性
            if link.startswith('/'):
            ...
            title_elem = element.xpath('./qyg2.js')  # 定位km-script标签
            if not title_elem:
                # 尝试其他可能的标题选择器
                title_elem = element.xpath('.//p[contains(@class, "script")]/text()')
                if not title_elem:
                    title_elem = element.xpath('./qyg3.js')
                    if not title_elem:
                        title_elem = element.xpath('./qyg4.js')
                        if not title_elem:
                            title_elem = element.xpath('./qyg5.js')
                            if not title_elem:
            ...
            pic_elem = element.xpath('./qyg6.js')  # 优先懒加载地址
            if not pic_elem:
                pic_elem = element.xpath('./qyg7.js')  # 兜底：直接src地址
        ...
        try:
            ...
            if play_links:
                ...
                for link in play_links:
                    ep_title = link.xpath('./qyg0.js')
                    ep_href = link.xpath('./qyg1.js')[0]
                    if ep_title:
